...
public class PhDDScholarsDetail extends AppCompatActivity {
    ...
    /**
     * Enter Name of PhD Scholars
     */
    private EditText edt_name_phd_scholar;
    private LinearLayout ll_name_phd_scholar;
    /**
     * Enter Department
     */
    ...
    String ID ="",status="";
    public  static  boolean is_back_phd = false;

    @Override
    protected void onCreate(Bundle savedInstanceState)
    {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_ph_ddscholars_detail);
        ...
        txt_act.setText("PhD Scholars Guided Detail");
        activity = PhDDScholarsDetail.this;
        initView();

        Intent intent = getIntent();

        ID = intent.getStringExtra("ID");
        System.out.println("Id of Get_PhD_Scholars_Guided_list from intent ::::::::::::::::::::::::: " + ID);
        status = intent.getStringExtra("status");

        if (ID != null && status != null && !ID.contentEquals("") && !status.contentEquals(""))
        {
            PhdSchollarsDetail(ID, status);
        }
    ...
    private void PhdSchollarsDetail(String id, String status)
    {
        ...
        queue = Volley.newRequestQueue(PhDDScholarsDetail.this);

        String url = URLS.Get_PhD_Scholars_Guided_list + "&user_id=" + mySharedPrefereces.getUserID() + "&emp_id=" + mySharedPrefereces.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + "1" + "&status=" + status + "&id=" + id + "";
...
//                DialogUtils.hideProgressDialog();
                ...
                System.out.println("Get_PhD_Scholars_Guided_list response size in leave listing :::::::::::::::::::::::::::::::::::::: " + response.length());
                if (response.length() > 10) {
                    ...
                    System.out.println("Get_PhD_Scholars_Guided_list approve leave listing data size :::::::::::::::::::::::::::::::::::::" + phdScolarGuidedPojo.getData().size());
                        ...
                        if (phdScolarGuidedPojo.getData() != null) {
                            if (phdScolarGuidedPojo.getData().get(0) != null) {
                                if (phdScolarGuidedPojo.getData().size() > 0) {
                                    ...
                                    edt_dept.setText(phdScolarGuidedPojo.getData().get(0).getDepartment()+"");
                                    edt_emp_name.setText(phdScolarGuidedPojo.getData().get(0).getEmployee_name() + "");
                                    edt_year.setText(phdScolarGuidedPojo.getData().get(0).getAcademic_Year());
                                    edt_name_phd_scholar.setText(phdScolarGuidedPojo.getData().get(0).getName_of_PhD_Scholars() + "");
                                    edt_title_of_reach_paper_domain.setText(phdScolarGuidedPojo.getData().get(0).getTitle_Domain_of_Research() + "");
                                    edt_year_awd.setText(phdScolarGuidedPojo.getData().get(0).getYear_of_Awarded() + "");
                                    edt_name_guide.setText(phdScolarGuidedPojo.getData().get(0).getName_of_Other_Guide() + "");
                                    edt_reg_name.setText(phdScolarGuidedPojo.getData().get(0).getYear_of_Registration() + "");
                                    edt_status.setText(phdScolarGuidedPojo.getData().get(0).getStatus() + "");
                                    edt_app_by.setText(phdScolarGuidedPojo.getData().get(0).getApprove_reject_by_user() + "");
                                    edt_app_date.setText(phdScolarGuidedPojo.getData().get(0).getApprove_rejct_date() + "");
                                    ...
                                    DialogUtils.Show_Toast(PhDDScholarsDetail.this, "No Records Found");
                                }
            ...
            public void onErrorResponse(VolleyError error) {
                DialogUtils.Show_Toast(PhDDScholarsDetail.this, "Please Try Again Later");
//                DialogUtils.hideProgressDialog();
    ...
    private  void ApproveConfarancePub(String reason, AlertDialog show,  final String ID,String app_rej)
    {
        ...
        DialogUtils.showProgressDialog(PhDDScholarsDetail.this,"");
        String url;
                            ...
                            if (conAppRejPojo!=null)
                            {
                                if (conAppRejPojo.getData()!=null)
                                {
                                    if (conAppRejPojo.getData().get(0)!=null&&conAppRejPojo.getData().size()>0)
                                    {
                                        ...
                                        {
                                            DialogUtils.Show_Toast(PhDDScholarsDetail.this,conAppRejPojo.getData().get(0).getMsg());
    ...
    private boolean isvalidate(EditText edt_reason) {
        if (edt_reason.getText().toString().trim().isEmpty() || edt_reason.getText().toString().contentEquals("") || edt_reason.getText().toString().length() < 0) {
            DialogUtils.Show_Toast(PhDDScholarsDetail.this, "Enter Reason");
            return false;
        }
    ...
    private void initView() {
        ...
        tv_approve.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showDialog(PhDDScholarsDetail.this,"approve",ID);
        ...
        tv_delete.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DialogUtils.showDialog4YNo(PhDDScholarsDetail.this, "", "Are You Sure To Reject ?", new DialogUtils.DailogCallBackOkButtonClick()
                {
                    @Override
                    public void onDialogOkButtonClicked()
                    {
                        showDialog(PhDDScholarsDetail.this,"reject",ID);
        ...
        edt_name_phd_scholar = (EditText) findViewById(R.id.edt_name_phd_scholar);
        ll_name_phd_scholar = (LinearLayout) findViewById(R.id.ll_name_phd_scholar);
        edt_dept = (EditText) findViewById(R.id.edt_dept);
        ll_dept = (LinearLayout) findViewById(R.id.ll_dept);
