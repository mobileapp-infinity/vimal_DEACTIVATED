...
public class DBConnector_2_10_19BK extends SQLiteOpenHelper {
...
// =====================================================================================
    public DBConnector_2_10_19BK(Context context) {
...
//        String sql = "SELECT * FROM " + TBL_GPS_DATA + " WHERE USER_ID='" + USER_ID + "' ORDER BY " + GPS_Master_Id +  " GROUP BY (GPS_DateTime)  LIMIT 50 ";
        String sql = "SELECT * FROM " + TBL_GPS_DATA + " WHERE USER_ID='" + USER_ID + "'  GROUP BY (GPS_DateTime)   ORDER BY " + GPS_Master_Id + "   LIMIT 50 ";
...
// =====================================================================================
    ...
    public List<GetAllSyncCityResponse.RECORD> getCityMaster() {
        String sql = "SELECT * FROM " + TBL_CITY_MASTER + " ORDER BY " + DBConnector_2_10_19BK.CITY_NAME;
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(sql, null);
    ...
    public int getCityID(String sourceCityName) {
        String citySelect = "SELECT " + DBConnector_2_10_19BK.CITY_ID + " FROM "
                + DBConnector_2_10_19BK.TBL_CITY_MASTER + " WHERE " + DBConnector_2_10_19BK.CITY_NAME + " = \""
...
//        String sql = "SELECT * FROM " + TBL_GPS_DATA + " WHERE USER_ID ='" + USER_ID + "' ORDER BY " + GPS_Master_Id + " LIMIT 50 ";
        //  String sql = "SELECT * FROM " + TBL_GPS_DATA + " WHERE USER_ID='" + USER_ID + "'  GROUP BY (GPS_DateTime)   ORDER BY " + GPS_Master_Id + "   LIMIT 50 ";
        String sql = "SELECT * FROM " + TBL_GPS_DATA + " WHERE USER_ID='" + USER_ID + "'  GROUP BY (GPS_DateTime)   ORDER BY " + GPS_Master_Id + "   LIMIT 50 ";
        System.out.println("getGPSMasterData query " + sql + "");
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(sql, null);
        List<GPSMasterBean> listGPSData = new ArrayList<>();
        ...
        try {
            if (cursor.getCount() > 0) {
                ...
                while (!cursor.isAfterLast()) {
                  ...
                  /*  if (data != null &&(!TextUtils.isEmpty(data.getGPS_Latitude())) && (!TextUtils.isEmpty(data.getGPS_Longitude())) &&
                            Double.parseDouble(data.getGPS_Latitude()) > 0 && Double.parseDouble(data.getGPS_Longitude()) > 0) {
                        listGPSData.add(data);
                    }*/
                   /* else if (data != null && (!TextUtils.isEmpty(data.getGPS_Status())) || data.getGPS_Status().trim().equals("0")) {
